...
ENV GOPRIVATE=*github.com/gobbeston

# Update OS package and install Git
RUN apk update && apk add git openssh && apk add build-base

# Set working directory
WORKDIR /go/src/github.com/gobbeston/trainning-golang-microservice
...
COPY --from=builder /go/src/github.com/gobbeston/trainning-golang-microservice/app/migrations/ migrations/
COPY --from=builder /go/pkg/mod/github.com/gobbeston/ /go/pkg/mod/github.com/gobbeston/

# Set Docker's entry point commands
RUN chown -R appuser:appgroup /go/pkg/mod/github.com/gobbeston/ /app
USER appuser
